...
<!-- Mini Charts: Donut + Top-5 Bar -->
<div class="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-4">
  <div class="col-span-1 rounded-2xl bg-white shadow-sm p-4">
    <div class="flex items-center justify-between mb-2">
      <h3 class="font-semibold text-slate-700">Total Forecast vs Actual</h3>
    </div>
    <div id="chart-donut-forecast-actual" class="h-64"></div>
  </div>
  <div class="col-span-2 rounded-2xl bg-white shadow-sm p-4">
    <div class="flex items-center justify-between mb-2">
      <h3 class="font-semibold text-slate-700">Top 5 Kuota Hampir Habis</h3>
      <a href="{{ route('admin.quotas.index') }}" class="text-xs text-sky-600 hover:underline">Lihat semua</a>
    </div>
    <div id="chart-bar-top5" class="h-64"></div>
  </div>
</div>
...
@php
  // Aggregate data for mini charts (no controller change)
  $q = isset($quotas) ? $quotas : (\App\Models\Quota::query()->where('is_active', true)->get());
  $totalAllocation = (int) $q->sum(fn($x) => (int)($x->total_allocation ?? $x->allocation ?? $x->qty_government ?? 0));
  $totalForecast   = (int) $q->sum(fn($x) => (int)($x->forecast_remaining ?? 0));
  $totalActual     = max($totalAllocation - $totalForecast, 0);
  $rows = $q->map(function($x){
      $alloc = (int)($x->total_allocation ?? $x->allocation ?? $x->qty_government ?? 0);
      $fcst  = (int)($x->forecast_remaining ?? 0);
      $name  = trim($x->name ?? $x->title ?? ($x->quota_number ?? 'Kuota'));
      return [
        'label' => $name,
        'allocation' => $alloc,
        'forecast' => $fcst,
        'remain_pct' => $alloc>0 ? round(($fcst/$alloc)*100) : 100,
      ];
    })
    ->sortBy('remain_pct')
    ->take(5)
...
(function(){
  if(!window.__apexInjected){
    var s=document.createElement('script'); s.src='https://cdn.jsdelivr.net/npm/apexcharts'; document.head.appendChild(s);
    window.__apexInjected=true;
  }
  function ready(fn){ document.readyState!=='loading'?fn():document.addEventListener('DOMContentLoaded',fn); }
  ready(function(){
    function mountDonut(){
      if(!window.ApexCharts){ setTimeout(mountDonut,200); return; }
      var el=document.querySelector('#chart-donut-forecast-actual'); if(!el) return;
      var totalActual={{ $totalActual }}; var totalForecast={{ $totalForecast }};
      try{
        var chart=new ApexCharts(el,{
          chart:{type:'donut',height:260},
          series:[totalActual,totalForecast],
          labels:['Actual','Forecast Remaining'],
          legend:{position:'bottom'},
          dataLabels:{enabled:true,formatter:(v)=>Math.round(v)+'%'},
          colors:['#22c55e','#94a3b8']
        });
        chart.render();
      }catch(e){ console.warn('Donut mount failed',e); }
    }
    function mountBarTop5(){
      if(!window.ApexCharts){ setTimeout(mountBarTop5,200); return; }
      var el=document.querySelector('#chart-bar-top5'); if(!el) return;
      var rows=@json($rows);
      var labels=rows.map(r=>r.label);
      var data=rows.map(r=>Math.max(0,100-(r.remain_pct||0)));
      try{
        var chart=new ApexCharts(el,{
          chart:{type:'bar',height:260,toolbar:{show:false}},
          series:[{name:'Konsumsi (%)',data:data}],
        ...
        chart.render();
      }catch(e){ console.warn('Bar mount failed',e); }
    }
    mountDonut();
    mountBarTop5();
  });
