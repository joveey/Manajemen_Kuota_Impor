...
class AllocBackfillActual extends Command
{
    ...
    public function handle(): int
    {
        ...
        $grQuery = DB::table('gr_receipts as gr')
            ...
            ->select(array_filter([
                $hasGrId ? 'gr.id as gr_id' : DB::raw('NULL as gr_id'),
                'gr.po_no',
                'gr.line_no',
                'gr.receive_date',
                'gr.qty',
        ...
        $handler = function($rows) use (&$scanned,&$written,&$skippedExisting,&$noQuota){
            foreach ($rows as $r) {
                $scanned++;
                ...
                $quota->decrementActual($qty, sprintf('Backfill GR %s/%s pada %s', $r->po_no, $r->line_no, $date), null, new \DateTimeImmutable($date), null, [
                    'gr_unique' => $uk,
                    'gr_id' => $r->gr_id ? (int)$r->gr_id : null,
        ...
        if ($hasGrId) {
            ...
            $grQuery->chunkById(500, $handler, 'gr.id', 'gr_id');
        } else {
            $grQuery
