            ...
            ImportItem::query()
                ...
                ->chunk(1000, function ($rows) use (&$applied, &$skipped, &$skippedExisting, &$duplicatesList, $now, $updateExisting) {
                    ...
                    foreach ($rows as $item) {
                        ...
                        $descLabel = trim((string)($norm['desc'] ?? ($raw['DESC'] ?? '')));
                    ...
                    if ($updateExisting) {
                        ...
                        $hasDescCol = DB::getSchemaBuilder()->hasColumn('hs_code_pk_mappings','desc');
                        foreach ($incoming as $code => $data) {
                            $row = [
                                'hs_code' => $code,
                                'pk_capacity' => $data['anchor'],
                                'created_at' => $now,
                                'updated_at' => $now,
                            ];
                            if ($hasPeriodCol) { $row['period_key'] = $period; }
                            if ($hasDescCol && !empty($data['desc'])) { $row['desc'] = (string) $data['desc']; }
                            $rowsUpsert[] = $row;
                        }
                        if ($hasPeriodCol) {
                            $updateCols = ['pk_capacity','updated_at']; if ($hasDescCol) { $updateCols[]='desc'; }
                            DB::table('hs_code_pk_mappings')->upsert($rowsUpsert, ['hs_code','period_key'], $updateCols);
                        } else {
                            $updateCols = ['pk_capacity','updated_at']; if ($hasDescCol) { $updateCols[]='desc'; }
                            DB::table('hs_code_pk_mappings')->upsert($rowsUpsert, ['hs_code'], $updateCols);
                        }
